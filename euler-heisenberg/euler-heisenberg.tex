...
%and, without loss of generality, $\vec{B}\parallel\vec{z}$.
One has
\begin{equation}
...
\rfun{\cos}{se\sqrt{\rbr{\vec B-\mi\vec E}^2}} \\
&= 4\rfun{\cos}{seB}\rfun{\cosh}{seE}\qquad
...
&=
\rbr{0, -x^0 E_i + \epsilon_{ijk} B^j x^k}.
\end{align}
applying which leads to \cref{eq:const-EM-strength}. In this choice the scalar 
part of \cref{eq:M_2-simp} is
\begin{align}
\rbr{\partial_{\mu} - \mi e A_\mu}^2 - m^2 = -\partial_0^2 +
\cbr{\partial_i - \mi e \rbr{-x^0 E_i + \epsilon_{ijk}B^j x^k}}^2 - m^2.
\end{align}

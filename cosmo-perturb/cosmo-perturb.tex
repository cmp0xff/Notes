...
Collecting the results above, one has
\begin{align}
&
\dva \mscrP^{ij} = M\,
\cbr{\varkappa\rbr{h^{ij}\mfrakF^{klmn}h_{mn}-4 \mfrakF^{ijkl}}h_{kl}
-\frac{\mfrakh^{1/2}}{2\varkappa}G^{ij} }
\nonumber \\
&+ M_{|k|l}\,\mfrakG^{ijkl} +
f_{kl}\,N \bigg\{
\varkappa \bigg[ -\frac{1}{2}\rbr{h^{ki}h^{jl}+h^{kj}h^{il}+h^{ij}h^{kl}}
\mfrakF^{mnpq}h_{mn}h_{pq}
\nonumber \\
&+
2\rbr{h^{ij}\mfrakF^{klmn}-h^{kl}\mfrakF^{ijmn}}h_{mn} - 4\mfrakF^{ijkl}
\bigg] - \frac{\mfrakh^{1/2}}{4\varkappa}\big(
R^{ij}h^{kl}-R^{ki}h^{jl}-R^{kj}h^{il}
\nonumber \\
&+
G^{kl}h^{ij}-G^{ki}h^{jl}-G^{kj}h^{il}\big)\bigg\}
+f_{kl|m}\,\bigg\{
N_{|n} \bigg[\frac{1}{2}\rbr{-\mfrakG^{ijmn}h^{kl}+\mfrakG^{ijkl}h^{mn}
-\mfrakG^{ijmk}h^{ln}-\mfrakG^{ijml}h^{kn}}
\nonumber \\
&-
\mfrakh^{1/2}\rbr{+h^{i(k}h^{l)(m}h^{n)j}-h^{i(k}h^{l)j}h^{mn}
+\rbr{ij\leftrightarrow mn}}\bigg]
-N_t\rbr{h^{tr}\mfrakp^{qp}+h^{tq}\mfrakp^{rp}-h^{tp}\mfrakp^{rq}}
\nonumber \\
&\cdot\big[\delta^{(i}{}_{r}\delta^{j)}{}_{s} \rbr{-
h^{sm}\delta^{k}{}_{q}\delta^{l}{}_{p}+
h^{sk}\delta^{l}{}_{q}\delta^{m}{}_{p}+
h^{sl}\delta^{m}{}_{q}\delta^{k}{}_{p}} +
\delta^{(i}{}_{r}\delta^{j)}{}_{q}\delta^{m}{}_{p}h^{kl}
\big] \bigg\}
\nonumber \\
&+
f_{kl|m|n}

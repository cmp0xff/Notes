...
h_{kp} \mfrakG_{qlmn} + \mfrakG_{klmp} h_{qn}}.
% \frac{1}{2\sqrt{\mfrakh}} \big(
% 2 \delta^i{}_{(n} \delta^j{}_{l)} h_{km} +
% 2 \delta^i{}_{(n} \delta^j{}_{k)} h_{lm} -
% \delta^i{}_k \delta^j{}_l h_{mn} -
% \delta^i{}_m \delta^j{}_n h_{kl} \big).
\end{align}
(check!)

Concerning the derivative of $\bar{\dva}u^{ij}{}_k$, one needs
\begin{align}
\bar{\dva}u^{ik}{}_{j|k} = \rbr{\dva h_{kl}}_{|m|n}\,
\frac{1}{2}\rbr{
h^{i[m} h^{k]n} \delta^l{}_j + h^{i[m}h^{l]n} \delta^k{}_j
+ \frac{1}{\sqrt{\mfrakh}} \mfrakG^{inkl} \delta^m{}_j},
\end{align}
so that
\begin{align}
h^{k(i}\,\bar{\dva}u^{j)l}{}_{k|l} &=
% \rbr{\dva h_{kl}}_{|m|n}\,\mfrakh^{-1/2}\rbr{
% h^{l(i}\mfrakG^{j)nmk} + h^{m(i}\mfrakG^{j)nkl}}.
\end{align}
